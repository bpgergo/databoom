...
create table export_budget2 as
select
f.id ||' - '||f.value cofog_name, 
ff.id ||' - '||ff.value parent_cofog_name, 
fff.id ||' - '||fff.value parent_parent_cofog_name,
e.id ||' - '||e.value econ_name, 
ee.id ||' - '||ee.value parent_econ_name, 
eee.id ||' - '||eee.value parent_parent_econ_name,
b.date_start, b.amount
from budget b
inner join func f on (f.id = b.func_id)
inner join econ e on (e.id = b.econ_id)
